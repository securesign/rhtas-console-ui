...
test.describe("Rekor Search UI", () => {
  ...
  test("Search by email", async ({ page }) => {
    ...
    await rekorSearchPage.search("Email", "carlosthe19916@gmail.com");
  ...
  test("Search by hash", async ({ page }) => {
    const rekorSearchPage = await RekorSearchPage.build(page);
    await rekorSearchPage.search("Hash", "sha256:1174c40f437d2cbd0d7d14fba2b030bf5595d63e66c26e784b764ba4a0bd6780");
  ...
  test("Search by log index", async ({ page }) => {
    const rekorSearchPage = await RekorSearchPage.build(page);
    await rekorSearchPage.search("Log Index", "380502080");
