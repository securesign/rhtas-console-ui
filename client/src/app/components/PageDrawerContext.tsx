...
const usePageDrawerState = () => {
  ...
  const [drawerPanelContent, setDrawerPanelContent] = React.useState<React.ReactNode>(null);
  const [drawerPanelContentProps, setDrawerPanelContentProps] = React.useState<Partial<DrawerPanelContentProps>>({});
...
export const PageContentWithDrawerProvider: React.FC<IPageContentWithDrawerProviderProps> = ({ children }) => {
  const pageDrawerState = usePageDrawerState();
  const { isDrawerExpanded, drawerFocusRef, drawerPanelContent, drawerPanelContentProps, drawerPageKey } =
    pageDrawerState;
  return (
    <PageDrawerContext value={pageDrawerState}>
      <div className={pageStyles.pageDrawer}>
        <Drawer isExpanded={isDrawerExpanded} onExpand={() => drawerFocusRef?.current?.focus()} position="right">
          <DrawerContent
            panelContent={
              <DrawerPanelContent
                isResizable
...
export const PageDrawerContent: React.FC<IPageDrawerContentProps> = ({
  ...
  const { setIsDrawerExpanded, drawerFocusRef, setDrawerPanelContent, setDrawerPanelContentProps, setDrawerPageKey } =
    React.use(PageDrawerContext);
  ...
  }, [children, drawerFocusRef, header, isExpanded, onCloseClick, setDrawerPanelContent]);
