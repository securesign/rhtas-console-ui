...
const AuthEnabledOidcProvider: React.FC<IOidcProviderProps> = ({ children }) => {
  const auth = useAuth();
  ...
  if (auth.error) {
    return (
      <Bullseye>
        ...
        >
          <EmptyStateBody>
            {`${auth.error.name}: ${auth.error.message}`}. Revisit your OIDC configuration or contact your admin.
          </EmptyStateBody>
        </EmptyState>
      </Bullseye>
    );
